...
- 操作系
  ...
  - 複数回: 全探索,貪欲法,dp
    - 操作の可換性がないか
  - 共通: グラフ,判定問題帰着
    - グラフ: 順序規定が難しく、制約が飛び地の場合
      - functional系
    - 判定問題帰着: 他のアプローチは全部無理そう,その値を達成可能かの判定は高速にできる
      - 最小の最大化
      - 複数の同時最適化
    - どのアプローチを取るにしても、全探索以外はまずエスパーする必要がある。
      - エスパーのコツ
        - 自由度が低い部分から考える
...
- ジャンル別
  - 区間が絡むタイプ
    - 尺取り法,累積和,fenwicktree,segtree,next配列
  - 剰余系
    - 和ならあらかじめ全要素modをとる
    - 積ならそれを含むかを考える(主客転倒的アプローチ)
  - 全ペアを走査するタイプ
    - 片方固定し、もう一方をアルゴリズムで高速に求める
- テクニック
  - 余事象
  - sortして良いならしておく
  - 自由度の低い部分から考える
...
- 確率/期待値の問題は、dpか組み合わせ論かの2つに大別される
  ...
  - 組み合わせ論の解き方
    - まず「集合」と「条件」を数学的に解釈する
    - 必要なら、数え上げの考え方を適用し、数える戦略を決める
      - 特に主客転倒は、期待値の線形性を利用したアプローチで極めて有効
    - P,C,余事象,包除原理,(dp) などの手法のどれを使うか検討する
- maxが何かの値に一致する確率を求めるより、maxが何かの値以下になる確率を求める方が遥かに簡単
  - 故に余事象を使う
- 「独立性」を考えることが最も重要
