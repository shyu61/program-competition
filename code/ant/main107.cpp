...
int main() {
    ...
    for (int i = 0; i < N; i++) {
        ...
        // 流量固定の場合は、ベルマンフォードを最初に実行せずともエッジを工夫することで全て非負にできる
        addEdge(v, u, 1, w[i]);  // あらかじめu->vに目一杯流す、つまり残余グラフは逆辺のみになる
        // 各ノードの流入出量は不変にする必要があるのでs,tから直接供給する
        addEdge(s, v, 1, 0);
        addEdge(u, t, 1, 0);
        // 逆向きにするために流したコスト分を先に加えておく
        res -= w[i];
    }

    // 負のエッジを持つ各ノードにおける流入出量は変えてはいけないので、s,tから直接供給することになる
    // 故にsから流すフローの総量はKに加えて、直接供給量Nも必要になる
