...
// - 部分一致を考える問題は非常に多い
//   - dp[i][j] := iは走査線, jは部分一致の状態(どこまで一致しているか) と設計する
//     - jを持たないと遷移が記述できない(末尾に新しく文字を追加したときにどうなるか？を遷移で書くので、前の文字列の状態が必要)
//     - jは「Tの何文字目まで一致したか」と考えるより「Tのprefix集合のうちどれと完全一致したか」と考える方がより汎用性が高い
...
int n, k;
string S;
vector<vector<int>> nxt;  // nxt[i][j] := i文字一致している状態で文字jを末尾に加えた時に何文字一致になるか
