...
// 方針
// クエリ問題の方針は以下の2つ
// - オンライン高速化: 各クエリの処理を事前計算により高速化(データ構造,二分探索が絡むことが多い <- logオーダーで計算できるため)
// - オフライン高速化:
//   - クエリをソートなりして順番に処理することで累積構造などを使った高速化を図る
//   - クエリをバッチ処理する
// いずれも事前計算が絡むことがほとんどで、何を事前計算するか？が鍵になるケースが殆ど
// -> まずオンライン高速化を考える。各クエリは逐次処理なので高速化には逐次処理を圧縮するしかないが、これがなかなか難しそう
// -> 方針切り替えてオフライン高速化を考える。累積構造を使うのは難しそう
// -> バッチ処理を考えると、区間更新(+1)だとわかる。lazysegtreeが使える形に持っていけないか考える。
// -> 区間更新すると区間値自体が変化してしまう。動的区間はsegtreeは扱えないので、更新するのは区間値ではなくあくまでノードの値と考えと、
// 更新するノードを二分探索で特定すれば良いことがわかる。(+1なら単調性が常に成り立つし、そもそもsegtree上では常に単調性が成立する)
// -> segtree上で二分探索は木構造をうまく使えばO(logN)でできるので十分高速に解ける
